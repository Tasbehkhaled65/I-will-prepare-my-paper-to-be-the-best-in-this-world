...
export default function Question_five() {
  ...
  const validationSchema = Yup.object().shape({
    // Add validation rules if needed for any form fields
    ticketId: Yup.string().required(),
  });

  const formik = useFormik({
    initialValues: {
      ticketId: '232', // Using the provided ticket ID
    },
    validationSchema,
    onSubmit: async (values) => {
      try {
        const response = await fetch(
          `https://lawngreen-walrus-394451.hostingersite.com/api/tickets/generate/${values.ticketId}`,
          {
            method: 'POST',
            headers: {
  ...
  // Determine text based on language (default to English)
  const t = texts[language] || texts.en;
  ...
  return (
    <form onSubmit={formik.handleSubmit} className={`flex xs:w-[270px] xs:flex-col xs:gap-[55px] xs:items-center relative w-full gap-[40px] p-[20px] ${
      darkMode ? 'bg-gray-800' : ''
    }`}>
      <div className='flex xs:h-[400px] h-[555px] w-[275px] items-center flex-col justify-between'>
        <div className='flex flex-col gap-[30px] items-center'>
      ...
      <div className={`w-[382px] xs:w-[300px] rounded-2xl flex gap-[50px] ${
        ...
        <img src={logo1} className='w-[230px] h-auto' alt="ticket" />
