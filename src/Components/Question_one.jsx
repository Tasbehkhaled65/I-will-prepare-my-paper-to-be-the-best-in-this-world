...
export default function Question_one() {
  ...
  useEffect(() => {
    const fetchTripData = async () => {
      try {
        const response = await fetch('https://lawngreen-walrus-394451.hostingersite.com/api/trip');
        if (!response.ok) {
    ...
    >
      ...
      <div className='flex xs:gap-[3px] gap-[32px]'>
        <div className={`w-[97px] flex items-center justify-center h-[48px] rounded-md gap-[15px] ${
          ...
          <p>{tripData?.departureTime || texts.time[language]}</p>
          <PiClockCountdownLight className={`text-lg ${
            darkMode ? 'text-gray-300' : 'text-white'
          }`} />
        </div>
      ...
      <div className='w-full flex flex-col gap-[50px]'>
        {tripData?.trainTypes?.map((trainType, index) => (
          <div key={index} className='flex flex-col w-full xs:gap-[15px] gap-[10px]'>
            <h1 className='font-bold text-xl'>
              {trainType.airConditioned 
                ? texts.trainTypes.airConditioned[language] 
                : texts.trainTypes.normal[language]}
            </h1>
            <div className='flex flex-col xs:gap-[10px]'>
              {trainType.classes?.map((trainClass, classIndex) => (
                <div key={classIndex} className='flex xs:justify-center xs:gap-[30px] font-semibold justify-between'>
                  <p>
                    {trainClass.name === 'First Class' 
                      ? texts.classes.firstClass[language] 
                      : texts.classes.secondClass[language]}
                  </p>
                  <p>{trainClass.price} {texts.currency[language]}</p>
                </div>
              ))}
            </div>
          </div>
