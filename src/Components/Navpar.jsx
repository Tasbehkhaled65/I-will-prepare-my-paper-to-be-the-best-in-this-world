  ...
  return (
    <div className={`bg-gray-800 text-white shadow-[0_4px_10px_rgba(0,0,0,0.3)] z-50 sticky top-0`}>
        ...
        <div className='flex items-center gap-4'>
          ...
          >
            ...
            <p 
              ...
              dangerouslySetInnerHTML={{ __html: translations[language].name }}
            />
          </button>
        </div>
      </div>
      ...
      <div className={`lg:hidden p-4 w-full relative font-Inter font-medium text-[18px] flex justify-between items-center ${language === 'ar' ? 'flex-row-reverse' : ''}`}>
        ...
        <div className={`bg-gray-800 w-[280px] h-screen fixed top-0 ${language === 'ar' ? 'left-0' : 'right-0'} z-50 shadow-2xl flex flex-col gap-4 p-6 transition-transform duration-300 ${
          small_div ? 'translate-x-0' : language === 'ar' ? '-translate-x-full' : 'translate-x-full'
        }`}>
          <button 
            onClick={() => setSmall_div(false)}
            className={`self-${language === 'ar' ? 'start' : 'end'} p-2 focus:outline-none`}
          >
            <HiXMark className='text-2xl text-red-500 hover:text-red-400' />
          </button>
